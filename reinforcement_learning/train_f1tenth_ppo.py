...
bootstrap_model = 'bs_models/HGDAgger_model.pkl.pkl'
...
def main():
    ...
    # Get IL bootstrap model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    bootstrap_il_agent = AgentPolicyMLP(1080, 256, 1, 0.001, device)
    bootstrap_il_agent.load_state_dict(torch.load(bootstrap_model))
    weight_temp = bootstrap_il_agent.state_dict()

    # Apply IL bootstrap model to initialize RL model
    model = PPO(**kwargs)
    model.actor.load_state_dict(weight_temp)
