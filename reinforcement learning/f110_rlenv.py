...
def create_f110env(**kargs):
    ...
    # with open('D:\Projects\il_bootstrap_rl\RL_example_change\RL_example\config\maps\config_levine_block.yaml') as file:
...
class F110Env_RL:
    ...
    def reset(self):
        ...
        theta_noise = (2*random.random() - 1) * 0.6
        theta = ((self.waypoints_xytheta[starting_idx][0, 2] - np.pi) + theta_noise) % (2 * np.pi)
    ...
    def get_obs(self, raw_obs: dict):
        ...
        obs = raw_obs['scans'][0]
        noise = np.random.normal(-0.25, 0.25, obs.shape[0])
        obs = obs + noise
